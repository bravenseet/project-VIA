"""
VIA Activity Categoriser — OpenRouter GUI Launcher
Run with: pythonw launcher_openrouter.pyw  (no console window)
       or: python launcher_openrouter.pyw
Create a Windows shortcut pointing to: pythonw "<full path>\launcher_openrouter.pyw"
"""
...
OPENROUTER_MODEL_DEFAULT = "google/gemini-2.0-flash-001"  # any slug from openrouter.ai/models
...
def filter_rows(raw: pd.DataFrame):
    total = len(raw)
...
def make_openrouter_categorizer(model: str, api_key: str):
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("VIA Activity Categoriser (OpenRouter)")
    ...
    def _build_ui(self):
        ...
        self.backend_var = tk.StringVar(value="openrouter")
        ...
        ttk.Radiobutton(backend_frame, text="OpenRouter (cloud API)",
                        variable=self.backend_var, value="openrouter",
        ...
        self.openrouter_frame = ttk.Frame(backend_frame)
        self.openrouter_frame.grid(row=3, column=0, sticky="w", pady=(6, 0))
        ttk.Label(self.openrouter_frame, text="  Model:").grid(row=0, column=0, sticky="w")
        self.openrouter_model_var = tk.StringVar(value=OPENROUTER_MODEL_DEFAULT)
        ttk.Entry(self.openrouter_frame, textvariable=self.openrouter_model_var, width=30).grid(row=0, column=1, padx=(4, 0))
        ttk.Label(self.openrouter_frame, text="  API Key:").grid(row=1, column=0, sticky="w", pady=(4, 0))
        self.api_key_var = tk.StringVar(value=os.environ.get("OPENROUTER_API_KEY", ""))
        ttk.Entry(self.openrouter_frame, textvariable=self.api_key_var, width=44, show="*").grid(
    ...
    def _browse_input(self):
        ...
        if path:
            ...
            if not self.output_var.get():
                self.output_var.set(os.path.join(os.path.dirname(path), "output.xlsx"))
    ...
    def _run(self):
        ...
        if not output_path:
            ...
            return
        if backend == "openrouter" and not self.api_key_var.get().strip():
    ...
    def _process(self, input_path, output_path, backend):
        try:
            ...
            if backend == "ollama":
                ...
                self._log(f"Categorising with OpenRouter / {model}...")
                try:
                    categorize_one = make_openrouter_categorizer(model, api_key)
                except ImportError:
                    self._log("ERROR: openai package not installed. Run: pip install openai")
                    self.after(0, lambda: self.run_btn.config(state="normal"))
                    return
